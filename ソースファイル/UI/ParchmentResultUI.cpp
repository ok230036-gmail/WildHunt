...
{
	// m_parchmentResultSpの初期設定
	m_parchmentResultSp = std::make_unique<SpriteCharacter>();

	m_parchmentResultSp->SetCameraLabel(L"HUDCamera");
	m_parchmentResultSp->SetGraphicsPipeLine(L"Sprite");
	m_parchmentResultSp->SetTextureId(L"ParchmentResult");
	...
	m_parchmentResultSp->SetPosition(0.0f, 0.0f, 0.0f);
	m_parchmentResultSp->SetScale(0.6f, 0.3f, 0.3f);


	m_parchmentResultSp->SetColorMix(SpriteCharacter::COLOR_MIX_OP::MIX_MUL);
	m_parchmentResultSp->SetColor(1, 1, 1, 1);
